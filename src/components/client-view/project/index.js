...
export default function ClientProjectView({ data }) {
  ...
  return (
    <div
      className="max-w-screen-xl mt-24 mb-6 sm:mt-14 sm:mb-14 pt-16 px-6 sm:px-8 lg:px-16 mx-auto"
      id="project"
    >
        ...
        <div className="flex flex-col justify-center items-center row-start-2 sm:row-start-1 ">
          <h1 className="leading-[70px] mb-4 text-3xl lg:text-4xl xl:text-5xl font-medium">
            {"My Projects".split(" ").map((item, index) => (
              <span
                className={`${index === 1 ? "text-blue-main" : "text-[#000]"}`} key={index}
              >
                {item}{" "}
              </span>
            ))}
      ...
      <AnimationWrapper>
        <ul className="project-wrapper" ref={containerRef}>
          {data && data.length
            ? data.map((item, index) => (
                <li
                  className="w-full flex items-stretch cursor-pointer shadow-lg "
                  key={index}
                >
                  <div className="border-2 w-full relative border-blue-main transition-all rounded-lg flex flex-col">
                    ...
                    <div className="absolute w-full bottom-0 justify-center flex gap-2">
                      <button onClick={()=>router.push(item.website)} className="p-2 text-white-500 font-semibold text-[14px] tracking-widest bg-blue-main transition-all outline-none rounded-md cursor-pointer hover:bg-slate-200">
                        Website
                      </button>
                      <button onClick={()=>router.push(item.github)} className="p-2 text-white-500 font-semibold text-[14px] tracking-widest bg-blue-main transition-all outline-none rounded-md cursor-pointer hover:bg-slate-200">
                        Github
                      </button>
                    </div>
                  </div>
